...
AccumulationLog::AccumulationLog() {
  auto fstream =
      std::make_shared<std::fstream>(fileName, std::ios::out | std::ios::app);
  init(fileName, level, fstream);
}
AccumulationLog::AccumulationLog(std::shared_ptr<std::fstream> fstream) {
  init(fileName, LogLevel::INFO, fstream);
}
AccumulationLog::AccumulationLog(LogLevel ll,
                                 std::shared_ptr<std::fstream> fstream) {
  init(fileName, ll, fstream);
}
AccumulationLog::AccumulationLog(const std::string& logFileName, LogLevel ll,
                                 std::shared_ptr<std::fstream> fstream) {
...
void AccumulationLog::init(const std::string& logFileName, LogLevel ll,
  ...
  if (!(*filePtr) || !filePtr->is_open()) {
    ...
    add(LogLevel::INFO, "LOG: Open at ", Timer::now());
  }
}
// No need for endl
...
void AccumulationLog::add(LogLevel ll, Args&&... args) {
  if (ll >= level) {
    ...
    (*filePtr) << addLogMessage(logLevelToString(ll), ": ",
...
void AccumulationLog::appendToStream(std::ostringstream& oss, T&& first,
                                     Args&&... args) {
...
void NormalAccumulationSpace::getVoxelListFromFile(
  ...
  for (auto l : vOrigins) {
    if (l.size() > 4) {
      voxelList.emplace_back(AccumulationVoxel({l[0][0], l[1][0], l[2][0]},
                                               l[3][0], l[4][0], 0, false));
      ...
      std::ostringstream oss;
      oss << " : P:" << accV.position << " | " << "votes:" << accV.votes << " "
          << "confs:" << accV.confidenceValue << " -> ";
...
std::pair<size_t, size_t> getMinMaxVotesCountFrom(
  ...
  auto minElem = std::min_element(
      accList.begin(), accList.end(),
      [&](const AccumulationVoxel& a, const AccumulationVoxel& b) {
...
std::pair<size_t, size_t> getMinMaxFacesCountFrom(
  ...
  auto faceSize = [](const AccumulationVoxel& accV) {
    return accV.associatedFaceIds.size();
  };
  auto minElem = std::min_element(
      accList.begin(), accList.end(),
      [&](const AccumulationVoxel& a, const AccumulationVoxel& b) {
...
size_t accumulationHash(DGtalPoint3D pos) {
  return std::hash<DGtalPoint3D>{}(pos);
}
