...
static FaceMap meshFacesMap; // face mapping to voxels
// static DequeList faceMap;
...
void initFacesMap()
{
    for (auto v : voxelList)
    {
        ...
        {
            size_t faceID = id;
            meshFacesMap.addValue(id, AccVoxelHelper::hash(v.p)); /*
                 std::cout << id << " face add" << v.p << std::endl;
             else
                 std::cout << id << " face not found or add error! " << std::endl; */
        }
    }
    meshFacesMap.print();
}
        ...
        for (auto const &f : listFace)
        {
            if (fVisited.count(f) == 0)
            {
                if ((getFaceBarycenter(polysurff, f) -
                     getFaceBarycenter(polysurff, faceId))
                        .norm() < radius)
                {
                    fVisited[f] = true;
                    result.push_back(f);
    ...
    if (io.MouseDoubleClicked[0])
    {
        ...
        if (nb > 0 && nb < vectSelection.size())
        {
            ...
            /*             auto voxelHash meshFacesMap.getValues(nb);
                        ...
                        // set some options
                        pt->setPointRadius(0.02);
                        pt->setPointRenderMode(polyscope::PointRenderMode::Quad); */
        }
    }
}
